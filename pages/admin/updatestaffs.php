<?php
  ...
  try{
    ...
    $get_email = $getDataRow['Email'];

    $sql_user = "SELECT * FROM users WHERE (email = '$get_email')";
      ...
      $Job = $_POST['job'];
        ...
        if(empty($FirstName)) throw new Error('First Name should not be empty');
          ...
          while($row = mysqli_fetch_array($result)) {           
            if($Email == $row['Email'] && $Contact == $row['Contact'] && $sn != $row['SN']) {
          ...
          if(isset($getDataRow_user)){
            $get_user_id = $getDataRow_user['id'];

            $updateSQL_user = "UPDATE users SET email = '$Email' WHERE (id = '$get_user_id')";
            mysqli_query($db, $updateSQL_user);
            $success_user = 'User with same email found and ';
          }
          else{
            $success_user = 'User with same email not found but ';
          }

          mysqli_query($db, $updateSQL);                
          $success = $success_user.'Staffs Details Updated Succesfully';
  ...
  <body class="dark">
    <div class="grid-section">
      ...
      <div class="right-section">
        ...
        <div class="content">
          <h2 class="heading_title">Update Staffs</h2>
            ...
            <div class="formgrid">
              ...
              <div class="forminput">
               <label for="firstName">First Name</label>
                <input name='firstName' type="text" class='search-area' placeholder="Enter First Name" autocomplete='off' value='<?php echo $getDataRow['FirstName'] ?>' />
              </div>

              <div class="forminput">
               <label for="lastName">Last Name</label>
                <input name='lastName' type="text" class='search-area' placeholder="Enter Last Name" autocomplete='off' value='<?php echo $getDataRow['LastName'] ?>' />
              </div>
              ...
              <div class="forminput">
               ...
               <label for="email">Email</label>
                <input name='email' type="email" class='search-area' placeholder="Enter Email" autocomplete='off' value='<?php echo $getDataRow['Email'] ?>' />
              </div>

              <div class="forminput">
               <label for="job">Job</label>
                <input name='job' type="text" class='search-area' placeholder="Enter Job" autocomplete='off' value='<?php echo $getDataRow['Job'] ?>' />
              </div>

              <div class="forminput">
               <label for="contact">Phone</label>
                <input name='contact' type="text" class='search-area' placeholder="Enter Contact Number" autocomplete='off' value='<?php echo $getDataRow['Contact'] ?>' />
              </div>

              <div class="forminput">
               <label for="address">Address</label>
                <input name='address' type="text" class='search-area' placeholder="Enter Address" autocomplete='off' value='<?php echo $getDataRow['Address'] ?>' />
              </div>
